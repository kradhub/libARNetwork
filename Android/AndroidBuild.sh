...
# $2 will always be the JNI install dir
...
if [ -z $PREFIX ] || [ -z $JNIDIR ] || [ -z $CONFIGURATION ]; then
	echo "Missing args !"
	exit 1
fi
...
CONF_ARGS="--prefix=$PREFIX --with-libARNetworkALInstallDir=$JNIDIR/libARNetworkAL --with-libARSALInstallDir=$JNIDIR/libARSAL --host=arm-linux-androideabi"
